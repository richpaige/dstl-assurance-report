...
be produced by the management strategy. Secondly, it considers the DO-178B and DO-178C objectives (and briefly touches on the Model-Based Development supplement to DO-178C) and analyses where specific objectives
...
and hence easier to follow. Next, we analyse the DO-178B and DO-178C objectives and summarise which specific objectives \textit{must} and
...
In the next section we briefly discuss the impact of migration on relevant safety standards -- particularly DO-178B and DO-178C -- and analyse the effect of migrating
a software system using the technical approach on any previously existing safety argument and safety process.

\section{Certification and DO-178B/C Objectives}
In this section we discuss issues associated with migrating a software application where the original system has been certified with respect to either DO-178B or DO-178C,
...
of migration on an accomplishment summary. Recall that DO-178B and DO-178C identify a set of software activities that can take place within the context of a broader 
...
DO-178B (DO-178C is largely similar at a high level; we consider the specific differences shortly) requires the following to be addressed:
...
The primary safety argument underlies the software life cycle data. This is clear in Annex A of DO-178B, which provides tables of objectives for each of ten areas (there are additional entries in objective tables for DO-178C). The tables are:
\begin{enumerate}
...
So if an accomplishment summary has been produced for a legacy system, and the software application has been migrated, then the accomplishment summary will no longer be valid and accurate for the migrated application. What parts of the accomplishment summary -- i.e., the Objective tables in DO-178B Annex A -- will either be invalidated (and hence require full work) or partially invalidated? We briefly outline the results of our initial analysis. We attempt to indicate if Objectives for DO-178B and DO-178C are \textit{directly} impacted by the application of the migration technology and strategy (and hence substantial work will be required to rebuild an accomplishment summary), and if Objectives are \textit{indirectly} impacted (in which case any rework will hopefully be minor). 

The assumptions that we make in carrying out this analysis are as follows.
\begin{itemize}
\item The original system has been developed according to DO-178B. Given that we are migrating legacy aerospace applications, at least in theory, this seems a reasonable assumption. In future, we should expect that development has been carried out according to DO-178C. 
...
\end{itemize}
We present two different but closely related analyses. The first is a preliminary assessment of the impact of migration purely against the DO-178B objectives. This takes into account projects where the migrated system is to be certified once again against DO-178B objectives. The second is a preliminary assessment of the impact of migration against DO-178C objectives. We also briefly touch on the Model-Based Development annexe. All of our analyses are preliminary, 
...
\subsection{DO-178B Assessment}
\label{sec:DO178B}
In this section we briefly analyse the impact of migration against DO-178B objectives.
